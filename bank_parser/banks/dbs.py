...
"""
DBS Bank India (digibank) statement parser.

e-Statements use ``Date | Transaction Details | Withdrawal | Deposit | Balance``
tables with ``DD/MM/YYYY`` rows. Because the withdrawal/deposit column that is
blank collapses out of the extracted text, every row typically shows the
transaction amount followed by the running balance; the debit/credit sign is
recovered from the running-balance arithmetic against the opening balance.
Narration that is too long to fit a row continues on the following lines.
"""
...
# A transaction row starts with a DD/MM/YYYY (or DD-MM-YYYY) date.
SAVINGS_ROW_RE = re.compile(r"^(\d{2}/\d{2}/\d{4}|\d{2}-\d{2}-\d{4})(?:\s+(.*))?$")

# Amounts (e.g. 27,995.99) anywhere in a row/block.
...
SKIP_RE = re.compile(
    ...
    r"Period[.:]|Account\s+(?:Number|No)|Account\s+Type|Branch|IFSC|Page\b|"
    r"Date\s+Transaction|Withdrawal|Deposit",
    re.IGNORECASE,
)

PERIOD_RE = re.compile(
    r"(?:Statement\s+)?Period[.:\s]*"
    r"(\d{2}/\d{2}/\d{4}|\d{2}-\d{2}-\d{4})\s*(?:to|-)\s*"
    r"(\d{2}/\d{2}/\d{4}|\d{2}-\d{2}-\d{4})",
    re.IGNORECASE,
)

ACCOUNT_RE = re.compile(r"Account\s+(?:Number|No)[.:\s]*([\dX*]+)")

OPENING_BALANCE_RE = re.compile(
    r"Opening\s+Balance[.:\s]*(?:INR\s*)?([\d,]+\.\d{2})", re.IGNORECASE
)
CLOSING_BALANCE_RE = re.compile(
    r"Closing\s+Balance[.:\s]*(?:INR\s*)?([\d,]+\.\d{2})", re.IGNORECASE
)
...
class DBSParser(BaseBankParser):
    ...
    def _extract_account_type(self, text: str) -> str | None:
        ...
        if "current" in lowered:
            return "Current"
        if "saving" in lowered or "digisavings" in lowered:
            return "Savings"
        return None
    ...
    def _classify_amount(
        ...
        if prev_balance is not None:
            ...
            if prev_balance - amount == balance:
                return amount, None
        if re.search(
            r"\bCr\b|REFUND|SALARY|DEPOSIT|INWARD|NEFT|RTGS|INTEREST", description, re.IGNORECASE
        ):
            return None, amount
        return amount, None
